...
export class GameApp {
  ...
  public async init(container: HTMLDivElement) {
    ...
    if (user && user.userId) {
      ...
      console.log('로그인 정보가 없어 게스트 모드로 진입합니다');
    }
  ...
  private update(ticker: Ticker) {
    ...
    if (isMoving) {
      ...
      // 멈춘 순간에는 즉시 전송
      this.sendMyPosition(false); // 움직임 False (혹은 그냥 마지막 위치)
    }
  ...
  public updateOtherPlayers(moves: MoveBroadcast[]) {
    ...
    // 이번 턴에 업데이트된 유저 ID 목록 (삭제 처리용으로 쓸 수도 있음)
    // const updatedUserIds = new Set<string>();

    moves.forEach((data) => {
      if (data.userId === this._myId) return; // 나 자신은 무시
      // updatedUserIds.add(data.userId);
      ...
      // 있으면 위치 이동
      if (otherPlayer) {
        // TODO: 나중에 여기에 Lerp(보간) 로직 추가 예정
        otherPlayer.x = data.x;
        otherPlayer.y = data.y;

        // 서버에서 isMoving을 안 주면, 이전 위치랑 비교해서 움직임 여부 판단해야 함
        // 지금은 일단 true로 둠 (방향은 서버가 줌)
        otherPlayer.setAnimation(data.direction, true);
      }
    });
  }
